...
if($_token && $_openid) {
	$_openid = decrypt($_openid, AJ_KEY.'QQID');
	...
	$pic = '';
	if($thumb) {
		if(strpos($thumb, AJ_PATH) === 0) {
			$pic = str_replace(AJ_PATH, AJ_ROOT.'/', $thumb);
		} else {
			if($AJ['remote_url'] && strpos($thumb, $AJ['remote_url']) === 0) {
				$pic = AJ_ROOT.'/file/temp/'.date('YmdHis', $AJ_TIME).mt_rand(10, 99).$_userid.'.'.$ext;
				file_copy($thumb, $pic);
			}
		}
		if($pic) {
